'''
# Equipment Failure Maintenance Multi-Label Classification

# Purpose
This Linear Support Vector Machine (LSVM) classifier categorizes failure maintenance strategies of of electric plant equipment.
...
class NLPModel():
    ...
    def makePlots(self):
        ...
        # Plot frequency distribution of 'Maintenance' strategy categories
        df = self.df
        dfMaintenance = df.drop(['Fleet', 'Station', 'Unit', 'UNID', 'Status', 'Function', 'System', 'Component', 'ComponentDesc', 'Subcomponent',
                                'SubcomponentDesc', 'SPVType', 'Derate%', 'Basis', 'Maintenance', 'Elimination', 'Classification', 'Manufacturer',
        ...
        numberOfOccurrences = []
        categories = list(dfMaintenance.columns.values)
        for category in categories:
            numberOfOccurrences.append(
                (category, dfMaintenance[category].sum()))
        dfStats = pd.DataFrame(
            numberOfOccurrences, columns=['Maintenance Strategy Category', 'Number of Occurrences in Train/Validate Set'])
        dfStats.plot(x='Maintenance Strategy Category', y='Number of Occurrences in Train/Validate Set',
                     kind='bar', legend=False, grid=True, figsize=(8, 5))
        plt.title("Maintenance Strategy Frequency in Train/Validate Set")
        plt.ylabel('Number of Occurrences in Train/Validate Set', fontsize=12)
        plt.xlabel('Maintenance Strategy Category', fontsize=12)

        # Plot distribution of the number of 'Maintenance' categories per record
        rowsums = df.iloc[:, 2:].sum(axis=1)
        x = rowsums.value_counts()
        plt.figure(figsize=(8, 5))
        plt.title(
            "Maintenance Strategies with Multiple Categories in Train/Validate Set")
        plt.ylabel('Number of Occurrences in Train Set', fontsize=12)
        plt.xlabel('Number of Categories', fontsize=12)
        plt.show()

        # Print count of the most frequent words in 'Maintenance' field
        print(Counter(" ".join(df["Maintenance"]).split()).most_common(100))

    def runModel(self, sampleIndex, sampleSize):
        '''
        Run the model on the given dataset (train/validate/apply) for the 'Maintenance' field
        '''
        df = self.df

        # Clean text for Maintenance
        df['Maintenance'] = df['Maintenance'].map(
        ...
        X_train = train.Maintenance
        X_test = test.Maintenance
        ...
        # Make 'Maintenance' category predictions
        categories = ['maintenance', 'operational', 'physical_barrier',
                      'design_and_engineering', 'supply_chain', 'unknown_Maintenance']
        predictions = []
        ...
        if(sampleIndex == 1):
            df = pd.DataFrame({'maintenance': predictions[0], 'operational': predictions[1], 'physical_barrier': predictions[2],
                               'design_and_engineering': predictions[3], 'supply_chain': predictions[4], 'unknown_Maintenance': predictions[5]})
            df.to_csv('./out/predictions.csv')


    def cleanText(self, text):
        '''
        Clean 'Maintenance' field text.
...
if __name__ == "__main__":
    '''
    Validate and then apply the model to predict 'Maintenance' field categories for entire dataset
    ...
    for sampleIndex, sampleSize in enumerate(sampleSizes):
        df = pd.read_csv(
            "./in/equipment_data_for_Maintenance.csv", encoding="ISO-8859-1")
        Model = NLPModel(df)
        Model.makePlots()
